...
def potential_interactions(shap_values_column, shap_values_matrix):
    ...
    ignore_inds = np.where(
        (shap_values_matrix.values.T - shap_values_column.values).T.std(0)
        < 1e-8
    )
    ...
    for i in range(X.shape[1]):
        encoded_val_other = encode_array_if_needed(
            X[inds, i][srt], dtype=np.float
        )
        ...
        if not (i in ignore_inds or np.sum(np.abs(val_other)) < 1e-8):
            for j in range(0, len(x), inc):
                if (
                    ...
                    v += abs(
                        np.corrcoef(
                            shap_ref[j : j + inc], val_other[j : j + inc]
                        )[0, 1]
                    )
        val_v = v
...
def encode_array_if_needed(arr, dtype=np.float64):
    try:
        ...
        encoding_dict = {
            string: index for index, string in enumerate(unique_values)
        }
        encoded_array = np.array(
            [encoding_dict[string] for string in arr], dtype=dtype
        )
...
def sample(X, nsamples=100, random_state=0):
    if nsamples >= X.shape[0]:
        ...
        return sklearn.utils.resample(
            X, n_samples=nsamples, random_state=random_state
        )
