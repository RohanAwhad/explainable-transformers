...
from ._general import format_value
...
def waterfall(shap_values, max_display=10, show=True):
    ...
    label_padding = np.array(
        [-0.1 * dataw if -w < 1 else 0 for w in neg_widths]
    )
    pl.barh(
        neg_inds,
    ...
    pl.axvline(
        fx, 0, 1, color="#bbbbbb", linestyle="--", linewidth=0.5, zorder=-1
    )
    ...
    tick_labels[0].set_transform(
        ...
        + matplotlib.transforms.ScaledTranslation(
            -10 / 72.0, 0, fig.dpi_scale_trans
        )
    )
    tick_labels[1].set_transform(
        tick_labels[1].get_transform()
        + matplotlib.transforms.ScaledTranslation(
            12 / 72.0, 0, fig.dpi_scale_trans
        )
    )
